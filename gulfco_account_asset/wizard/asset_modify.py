...
class AssetModify(models.TransientModel):
    ...
    def transfer(self):
        for record in self:
            transfer_dic= {}
            for move in self.env['account.move'].search([('date', '>=', record.date),
                                                         ('asset_id', '=', record.asset_id.id)]):
                if move.state != 'draft':
                    raise UserError(_("There is some posted entries within selected date"))
                for move_line in move.line_ids:
                    move_line.analytic_distribution = record.analytic_distribution
                    record.asset_id.from_analytic_distribution = record.asset_id.analytic_distribution

            record.asset_id.transfer = True
            record.asset_id.transfer_date = record.date
            record.asset_id.transfer_memo = record.name
            record.asset_id.analytic_distribution = record.analytic_distribution
            record.asset_id.to_analytic_distribution = record.analytic_distribution
            transfer_dic.update({'asset_id': record.asset_id.id,
                                 'original_location': record.asset_id.from_analytic_distribution,
                                 'new_location': record.analytic_distribution,
                                 'transfer_date': record.date,
                                 'transfer_memo': record.name,
                                 'transfer_by': self.env.user.id, })
            self.env['account.asset.transfer'].create(transfer_dic)
    ...
    def modify(self):
        ...
        if not self.env.context.get('resume_after_pause'):
            if self.env['account.move'].search_count([('asset_id', '=', self.asset_id.id), ('state', '=', 'draft'), ('date', '<=', self.date)], limit=1):
                print(self.env['account.move'].search([('asset_id', '=', self.asset_id.id), ('state', '=', 'draft'), ('date', '<=', self.date)]),"11111111111111111111111111111111111111111111111111111111")
        ...
        if self.currency_id.compare_amounts(residual_increase + salvage_increase, 0) > 0:
            ...
            asset_increase = self.env['account.asset'].create({
                ...
                'original_move_line_ids':  [(6, 0, self.env.context.get('revaluation_move').invoice_line_ids.filtered(lambda r: r.account_id == self.account_asset_id).ids)] if 'revaluation_move' in self.env.context else [(6, 0, move.line_ids.filtered(lambda r: r.account_id == self.account_asset_id).ids)],
            })
