...
class FSMRecurringOrder(models.Model):
    ...
    partner_team_member_ids = fields.Many2many('res.partner', 'fsm_recurring_members_rel', string='Team Members',
                                               compute="_get_team_member", store=True)
    vehicle_id = fields.Many2one('fleet.vehicle',related="person_id_partner.vehicle_id",store=True)
