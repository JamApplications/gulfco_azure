...
class FSMRoute(models.Model):
    ...
    crm_team_id = fields.Many2one(comodel_name="crm.team", string="Team")
    day_ids = fields.Many2many(comodel_name="fsm.route.day", string="Days", required=True)
    max_order = fields.Integer(
        string="Maximum Orders",
        default=0,
        help="Maximum number of orders per day route.",
    )
    location_ids = fields.Many2many('fsm.location',string="Locations")
