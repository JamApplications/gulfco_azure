...
class FSMOrder(models.Model):
    ...
    dayroute_id = fields.Many2one(
        comodel_name="fsm.route.dayroute", string="Journey", index=True
    )
    ...
    customer_visits = fields.Integer(string="No. Of Customer visits", compute="_compute_early_visits",
                                     store=True)
        ...
        try:
            for vals in vals_list:
                if not vals.get("fsm_route_id") and vals.get("location_id"):
                    ...
                    vals.update({"fsm_route_id": location.fsm_route_ids[0].id})
    ...
    @api.depends('customer_id', 'scheduled_date_start')
    def _compute_early_visits(self):
        for record in self:
            if not record.customer_id or not record.scheduled_date_start:
                record.customer_visits = 0
                continue
            else:
                early_visits = self.search_count([
                    ('customer_id', '=', record.customer_id.id),
                    ('scheduled_date_start', '<', record.scheduled_date_start),
                    ('id', '!=', record.id)
                ])
                record.customer_visits = early_visits
