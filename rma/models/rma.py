...
class Rma(models.Model):
    ...
    picking_id = fields.Many2one(
        ...
        domain=(
            "["
            "    ('state', 'not in', ['draft', 'cancel', 'planned', 'assigned', 'confirmed']),"
    ...
    def _check_qty(self):
        for rma in self:
            if (rma.product_uom_qty > rma.move_id.remaining_qty) and rma.move_id:
                raise ValidationError(_("RMA quantity can't be greater than move remaining quantity quantity"))
    ...
    replace_product_id = fields.Many2one(comodel_name="product.product", string="Replacing product")
    replace_product_uom_qty = fields.Float(
        string="Replacing product quantity",
        digits="Product Unit of Measure",
    )

    @api.depends('reception_move_id')
    def _compute_reception_count(self):
        for rma in self:
            ...
            while picking:
                if len(picking) == 1:
                    next_picking_ids.append(picking.id)
                    picking = picking._get_next_transfers()
                else:
                    for p in picking:
                        next_picking_ids.append(p.id)
                        picking = picking._get_next_transfers()
            rma.reception_count = len(next_picking_ids)
    ...
    def _compute_delivery_picking_count(self):
        for record in self:
            ...
            record.delivery_picking_count = len(next_picking_ids)
    ...
    @api.depends("picking_id", "product_id", "company_id", "user_id")
    def _compute_location_id(self):
        for record in self:
            if record.picking_id and not record.user_id:
                warehouse = record.picking_id.picking_type_id.warehouse_id
                record.location_id = warehouse.rma_loc_id.id
            elif record.picking_id and record.user_id.user_type != 'sales_user':
                warehouse = record.picking_id.picking_type_id.warehouse_id
                record.location_id = warehouse.rma_loc_id.id
            elif record.user_id.user_type == 'sales_user' and record.user_id.partner_id.van_location:
                record.location_id = record.user_id.van_location.id
    ...
    def _send_draft_email(self):
        ...
        for rma in self.filtered("company_id.send_rma_draft_confirmation"):
            ...
            email_template.send_mail(rma.id, force_send=True)
    ...
    def action_confirm(self):
        for record in self:
            ...
            if record.state == "draft" or record.env.context.get('from_rma_extended') or record.user_id.user_type == 'sales_user':
                if not record.reception_move_id:
                    ...
                    record.sudo().write({"reception_move_id": reception_move[0].id, "state": "confirmed"})
                    record.sudo().write({"reception_move_ids": reception_move.ids,})
                    if record.user_id.user_type != 'sales_user':
    ...
    def action_refund(self):
        ...
        _logger.info("SHOOOOOO")
        _logger.info(group_dict)
        for rmas in group_dict.values():
            ...
            _logger.info("SHOOOOOOOOOOOOOOOOOO")
            _logger.info(refund_vals)
            _logger.info("SHOOOOOOOOOOOOOOOOOO")
            today = fields.Date.context_today(self.env.user).strftime("%Y-%m-%d")
            suffix = f" - {today}"
            new_ref = (refund_vals['ref'] or "")
            refund_vals['ref'] = new_ref + "-" + suffix
            refund = self.env["account.move"].with_context(mail_create_nosubscribe=False).sudo().create(refund_vals)
            if not refund.invoice_line_ids and self.env.context.get('from_rma_server_action', False):
                refund.write({'invoice_line_ids': refund_vals.get('invoice_line_ids')[0]})
            refund.action_post()

            for rma in rmas:
                if rma.invoice_id:
                    # Reconcile payment with invoice
                    invoice_receivable_line = rma.invoice_id.line_ids.filtered(
                        lambda l: l.account_id.account_type == 'asset_receivable'
                    )
                    refund_receivable_line = refund.line_ids.filtered(
                        lambda l: l.account_id.account_type == 'asset_receivable'
                    )

                    if invoice_receivable_line and refund_receivable_line:
                        (invoice_receivable_line + refund_receivable_line).reconcile()
    ...
    def action_view_receipt(self):
        ...
        if len(next_picking_ids) == 1:
            ...
            action.update(
                view_mode="list,form",
                domain=f"[('id', 'in', {next_picking_ids})]"
            )
    ...
    def action_view_delivery(self):
        ...
        if len(next_picking_ids) > 1:
            action["domain"] = [("id", "in", next_picking_ids)]
            action["view_mode"] = "list,form"
        else:
            action.update(
                res_id=next_picking_ids[0],
                view_mode="form",
                view_id=False,
                views=False,
            )
    ...
    def _create_receptions_from_product(self):
        self.ensure_one()
        picking = self.env["stock.picking"].sudo().create(self._prepare_picking_vals())
    ...
    def extract_quantity(self, qty, uom):
        ...
        extracted_rma.suod().message_post_with_source(
            "mail.message_origin_link",
    ...
    def _prepare_refund_vals(self, origin=False):
        ...
        if self.user_id and self.user_id.user_type == 'sales_user':
            ...
            rma_vals['journal_id'] = journal_id
            # rma_vals['responsible_id'] = self.sales_person_id.id
            # rma_vals['custodian_id'] = custodian_id.id
        ...
        rma_vals = {
            ...
            "invoice_line_ids": [],
            "invoice_user_id": self.user_id.id if self.user_id else False,
            "assign_to": self.sales_person_id.id if self.sales_person_id else False,
    ...
    def create_return(self, scheduled_date, qty=None, uom=None):
        ...
        for rmas in grouped_rmas:
            ...
            picking.sudo().action_confirm()
            picking.sudo().action_assign()
            picking.sudo().message_post_with_source(
                "mail.message_origin_link",
    ...
    def _action_launch_stock_rule(
        ...
        if not self.procurement_group_id:
            self.procurement_group_id = (
                self.env["procurement.group"].sudo()
                .create(
                    {
                        "name": self.name,
                        "move_type": "direct",
    ...
    def update_received_state_on_reception(self):
        ...
        """
        self.sudo().write({"state": "product_under_inspection"})
    ...
    def update_returned_state(self):
        ...
        if rma:
            rma.sudo().write({"state": "returned"})

    def write(self, values):
        res = super(Rma, self).write(values)
        if values.get("state") in ('received','replaced') and self:
            self.move_id.remaining_qty -= self.product_uom_qty
        return res
