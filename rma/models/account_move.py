...
class AccountMove(models.Model):
    ...
    def write(self, vals):
        ...
        for rec in self:
            previous_state = old_states.get(rec.id)
            new_state = rec.state

            if rec.move_type == 'out_refund' and rec.invoice_line_ids.filtered(lambda l: l.rma_id):
                template_id = False
                if previous_state == 'draft' and new_state == 'posted':
                    template_id = self.env.ref('rma.mail_template_credit_note_posted', raise_if_not_found=False)
                elif previous_state != 'cancel' and new_state == 'cancel':
                    template_id = self.env.ref('rma.mail_template_credit_note_cancelled', raise_if_not_found=False)

                if template_id and rec.partner_id.email:
