...
class PortalRma(CustomerPortal):
    ...
    def _prepare_stock_moves_for_portal(self, rma):
        reception_move = rma.reception_move_id[-1].picking_id if rma.reception_move_id else None
        delivery_move = rma.delivery_move_ids.mapped("picking_id") if rma.delivery_move_ids else None
        if delivery_move:
            while delivery_move._get_next_transfers():
                delivery_move = delivery_move._get_next_transfers()
        return reception_move, delivery_move


    def _rma_get_page_view_values(self, rma, access_token, **kwargs):
        reception_move, delivery_move = self._prepare_stock_moves_for_portal(rma)
        values = {
            "page_name": "rma_form",
            "rma": rma,
            "reception_move": reception_move,
            "delivery_move": delivery_move,
        }
    ...
    def portal_my_rma_picking_report(self, rma_id, picking_id, access_token=None, **kw):
        ...
        pdf = request.env['ir.actions.report'].sudo()._render_qweb_pdf("stock.action_report_delivery",res_ids=picking_sudo.id)[0]
        pdfhttpheaders = [
