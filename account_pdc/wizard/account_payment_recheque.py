...
class AccountPaymentReCheque(models.TransientModel):
    ...
    pdc_bank_id = fields.Many2one('res.bank', string="Cheque Bank", required=True,)
        ...
        if payment.is_pdc_payment:
            ...
            if reconciled_invoice_ids:
                vals = {
                    ...
                    # 'pdc_ref': self.cheque_number, (comment code because of pdf_Ref not exist)
                ...
                vals = {
                    'amount': payment._get_payment_amount(company_currency=False),
                    'memo': self.cheque_number,
                    'pdc_ref': self.cheque_number,
                    'currency_id': payment.currency_id.id,
                    'payment_type': 'inbound',
                    'partner_type': 'customer',
                    'partner_id': payment.partner_id.id,
                ...
                new_payment.move_id.cheque_payment_type = 'recheque'
            ...
            # self._send_notification_to_accounting(payment,new_payment)
    ...
    # def _send_notification_to_accounting(self,payment,new_payment):
    ...
    #                 user_id=user.id,
    #                 summary="Notification for the {} Payment is Rechequed with {} new payment".format(payment.name,new_payment.name),
    #                 note="",
    #             )