...
class account_payment(models.TransientModel):
    ...
    is_exchange = fields.Boolean('Apply Manual Exchange')
    rate = fields.Float('Rate', digits=(12, 12), tracking=500)
    ...
    def default_get(self, default_fields):
        ...
        if len(invoices) == 1:
            rec.update({
                'is_exchange': invoices.is_exchange,
                'rate': invoices.rate
            })
        return rec
        ...
        for active_id in active_ids:
            if active_id in account_move:
                ...
                for invoice in invoices:
                    rec.update({
                        'is_exchange': invoice.is_exchange,
                        'rate': invoice.rate
                    })
    ...
    def _compute_amount(self):
        for wizard in self:
            ...
            if wizard.source_currency_id == wizard.currency_id:
                ...
                if wizard.is_exchange and wizard.rate:
                    wizard.amount = wizard.source_amount_currency * wizard.rate
                else:
    ...
    @api.depends('can_edit_wizard', 'amount', 'installments_mode', 'is_exchange', 'rate')
    def _compute_payment_difference(self):
        for wizard in self:
            ...
            if wizard.source_currency_id == wizard.currency_id:
                ...
                if wizard.is_exchange and wizard.rate:
                    wizard.payment_difference = (wizard.source_amount_currency * wizard.rate) - wizard.amount
                else:
                    amount_payment_currency = wizard.company_id.currency_id._convert(
                        wizard.source_amount, wizard.currency_id, wizard.company_id, wizard.payment_date)
    ...
    def _create_payment_vals_from_wizard(self, batch_result):
        ...
        if self.is_exchange:
            res.update({'is_exchange': self.is_exchange,
                        'rate': self.rate})
        return res
...
class AccountPayment(models.Model):
    ...
    def _compute_payment_amount(self, invoices, currency, journal, date):
        ...
        self._cr.execute('''
            ...
            GROUP BY move.id, move.type, move.currency_id
        ''', [tuple(invoices.ids)])
        ...
        for inv in invoices:
            for res in query_res:
                ...
                if move_currency == currency and move_currency != company.currency_id:
                    ...
                    if not inv.is_exchange:
                        total += company.currency_id._convert(res['amount_residual'], currency, company, date)
                    else:
                        total += res['residual_currency'] * inv.rate
        return total

    @api.depends('invoice_ids', 'amount', 'payment_date', 'currency_id', 'payment_type', 'rate', 'is_exchange')
    def _compute_payment_difference(self):
        ...
        for pay in draft_payments:
            ...
            pay.payment_difference = pay._compute_payment_amount(
                pay.invoice_ids, pay.currency_id, pay.journal_id, pay.payment_date) - payment_amount
    ...
    def _prepare_move_line_default_vals(self, write_off_line_vals=None, force_balance=None):
        ...
        if self.is_exchange and self.rate:
            liquidity_balance = liquidity_amount_currency * self.rate
            write_off_balance = write_off_amount_currency * self.rate
            counterpart_amount_currency = -liquidity_amount_currency - write_off_amount_currency
            counterpart_balance = -liquidity_balance - write_off_balance
            currency_id = self.currency_id.id
        elif self.is_exchange and not self.rate:
            ...
            liquidity_balance = self.currency_id._convert(
                liquidity_amount_currency, self.company_id.currency_id,
                self.company_id, self.date)
            counterpart_amount_currency = -liquidity_amount_currency - write_off_amount_currency
            counterpart_balance = -liquidity_balance - write_off_balance
            currency_id = self.currency_id.id
