...
class StockRequestOrder(models.Model):
    ...
    picking_count = fields.Integer(
        string="Delivery Orders", compute="_compute_count_picking_ids", readonly=True
    )
    ...
    @api.depends("warehouse_id", "location_id", "stock_request_ids","to_warehouse_id")
    def _compute_route_ids(self):
        ...
        for record in self:
            ...
            if record.stock_request_ids:
                ...
                if record.direction == 'sample_issue_out':
                    final_routes = final_routes.filtered(lambda s:s.is_delivery_route)
                elif record.direction in ['foc_receiving']:
                    final_routes = final_routes.filtered(lambda s:s.is_receive_route)
                elif record.direction == 'branch_transfer' or (record.direction == 'consumable_issuance' and record.consumable_issuance_type == 'branch'):
                    final_routes = final_routes.filtered(lambda s:s.supplied_wh_id == record.warehouse_id)
                record.route_ids = [(6, 0, final_routes.ids)]
            else:
                if record.direction == 'sample_issue_out':
                    filtered_routes = filtered_routes.filtered(lambda s: s.is_delivery_route)
                elif record.direction in ['foc_receiving']:
                    filtered_routes = filtered_routes.filtered(lambda s: s.is_receive_route)
                elif record.direction == 'branch_transfer' or (record.direction == 'consumable_issuance' and record.consumable_issuance_type == 'branch'):
                    filtered_routes = filtered_routes.filtered(lambda s:s.supplied_wh_id == record.warehouse_id)
    ...
    def _compute_state(self):
        for item in self:
            ...
            if not item.stock_request_ids or all(x == "draft" for x in states):
                ...
                item.state = "done"
            elif all(x in ("submitted") for x in states):
                item.state = "submitted"
            else:
                item.state = "open"

    @api.depends("picking_ids","stock_request_ids.allocation_ids")
    ...
    def action_confirm(self):
        for rec in self:
            if rec.direction != 'stock_takeover' and not rec.stock_request_ids:
                raise UserError(
                    _("There should be at least one request item for confirming the order.")
                )
            rec.mapped("stock_request_ids").action_confirm()
        return True

    def action_reject(self):
        self.mapped("stock_request_ids").action_reject()
        for rec in self:
    ...
    def change_direction_warehouse(self):
        for rec in self:
            if rec.direction == "van_load" and rec.warehouse_id and rec.warehouse_id.van_load_route_id:
                rec.route_id = rec.warehouse_id.van_load_route_id.id
            elif rec.direction == "van_off_load" and rec.warehouse_id.van_offload_route_id:
                rec.route_id = rec.warehouse_id.van_offload_route_id.id
            else:
                rec.route_id = False

    @api.constrains("direction","warehouse_id")
    def const_direction_warehouse(self):
        for rec in self:
            if rec.direction == "van_load" and rec.warehouse_id and rec.warehouse_id.van_load_route_id:
                rec.route_id = rec.warehouse_id.van_load_route_id.id
            elif rec.direction == "van_off_load" and rec.warehouse_id.van_offload_route_id:
