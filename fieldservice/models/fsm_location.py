...
class FSMLocation(models.Model):
    ...
    # _rec_names_search = ["location_name", "complete_name"]
    _rac_name = "location_name"
    ...
    child_ids = fields.Many2many('res.partner', 'fsm_location_partner_child_rel')

    @api.onchange('partner_id')
    def _onchange_get_child_ids(self):
        if self.partner_id:
            self.child_ids = [(6,0, self.partner_id.child_ids.ids)]
    ...
    owner_id = fields.Many2one(
        ...
        ondelete="restrict",
        auto_join=True,
        domain="['|',('id', '=', partner_id),('id', 'in', child_ids)]"
    )
    ...
    def name_search(self, name='', args=None, operator='ilike', limit=100):
        args = args or []
        if name:
            args = ['|', ('location_name', operator, name), ('complete_name', operator, name)] + args
        recs = self.search(args, limit=limit)
        return recs.name_get()

    @api.depends("location_name","partner_id.name", "fsm_parent_id.complete_name", "ref")
            ...
            if loc.fsm_parent_id:
                if loc.ref:
                    loc.complete_name = (
                        ...
                        f"[{loc.ref}] {loc.location_name}"
                    )
                else:
                    loc.complete_name = (
                        f"{loc.fsm_parent_id.complete_name} / {loc.location_name}"
                    )
            else:
                if loc.ref:
                    loc.complete_name = f"[{loc.ref}] {loc.location_name}"
                else:
                    loc.complete_name = loc.location_name
