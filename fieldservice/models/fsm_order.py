...
class FSMOrder(models.Model):
    ...
    route_status = fields.Selection(
        selection=[("inRoute", "In Route"), ("outRoute", "Out Route")],
        string="Route Status",
    ...
    rel_route_status = fields.Selection(related='route_status', store=True)
    ...
    # person_ids_a = fields.Many2many("res.partner", string="Field Service Workers", domain=_get_sales_team_domain) #domain="[('team_id', 'in', team_id)]")
    ...
    person_id_partner = fields.Many2one("res.partner", string="Assigned To",) # domain="[('id', 'in', sales_team_id.partner_member_ids.ids)]") #domain=_get_sales_team_domain)
    ...
    scheduled_duration = fields.Float(related='template_id.duration', store=True, help="Scheduled duration of the work in" " hours")
    ...
    # Template
    template_id = fields.Many2one("fsm.template", string="Activities")
    ...
    def _get_default_vehicle(self):
        return self.person_id_partner.vehicle_id.id or False

    vehicle_id = fields.Many2one(
        "fleet.vehicle", string="Vehicle", default=_get_default_vehicle
    )

    @api.model_create_multi
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get("vehicle_id") and vals.get("person_id_partner"):
                self._onchange_person_id()
        return super().create(vals_list)

    @api.onchange("person_id_partner")
    def _onchange_vehicle_person_id_partner(self):
        self.vehicle_id = (
                self.person_id_partner
                and self.person_id_partner.vehicle_id
                and self.person_id_partner.vehicle_id.id
                or False
        )

    #Add new field named customer after template field (activities).
    ...
    is_start = fields.Boolean(default=True)
    ...
    def action_complete(self):
        ...
        # company_timezone = self.env.user.tz  # Get the user's timezone
        # localized_date = fields.Datetime.context_timestamp(self, current_date)
        # print("localized_date:", localized_date)
        return self.write(
            {
    ...
    def action_start(self):
        current_date = fields.Datetime.now()
        print("current_date:", current_date)
        if self.route_status in ['outRoute']:
            # raise ValidationError("Start Action restricted as visit is in OutRoute State.")
            raise UserError("Action cannot be started as the visit is currently in the 'OutRoute' state. ")
    ...
    def _onchange_template_team_id(self):
        if self.team_id:
            template_id = self.env['fsm.template'].search([('team_id', '=', self.team_id.id)])
            self.template_id = template_id.id or None
    ...
    def _compute_route_status(self):
        ...
        print("today:", today)
        for record in self:
            if record.scheduled_date_start and record.scheduled_date_start.date() == today:
                record.route_status = 'inRoute'  # Green (In Route)
            else:
                record.route_status = 'outRoute'  # Red (Out Route)

    @api.model
    def cron_update_route_status(self):
        """Optimized route status update for production environments"""
        import time
        start_time = time.time()
        today = date.today()

        try:
            _logger.info(f"Route status cron starting - today is: {today}")

            # Use database-level update for maximum performance
            # Update records with today's date to inRoute
            self.env.cr.execute("""
                UPDATE fsm_order
                SET route_status = 'inRoute', rel_route_status = 'inRoute'
                WHERE DATE(scheduled_date_start) = %s
                AND (route_status != 'inRoute' OR route_status IS NULL)
            """, (today,))

            inroute_count = self.env.cr.rowcount
            _logger.info(f"Set {inroute_count} records to inRoute")

            # Update records NOT with today's date to outRoute
            self.env.cr.execute("""
                UPDATE fsm_order
                SET route_status = 'outRoute', rel_route_status = 'outRoute'
                WHERE (scheduled_date_start IS NULL OR DATE(scheduled_date_start) != %s)
                AND (route_status != 'outRoute' OR route_status IS NULL)
            """, (today,))

            outroute_count = self.env.cr.rowcount

            # Commit the transaction
            self.env.cr.commit()

            elapsed_time = time.time() - start_time
            _logger.info(f"Route status cron completed successfully in {elapsed_time:.2f}s - inRoute: {inroute_count}, outRoute: {outroute_count}")

        except Exception as e:
            _logger.error(f"Route status cron failed: {e}")
