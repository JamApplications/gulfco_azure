...
class AccountHandOverCdc(models.Model):
    _name = 'account.hand.over.cdc'
    _description = 'Account Hand Over CDC'
    ...
    payment_ids = fields.Many2many(
        ...
        required=True,
        check_company=True,
        domain="[('is_cdc_payment', '=', True),('cheque_owner_id', '=', user_from_id),('cdc_state', '=', 'registered')]",
    )
    ...
    def create(self, vals_list):
        for values in vals_list:
            ...
            if values.get('name', _('New')) == _('New'):
                ...
                values['name'] = self.env['ir.sequence'].next_by_code('account.hand.over.cdc',
                                                                      sequence_date=seq_date) or _('New')
        return super(AccountHandOverCdc, self).create(vals_list)

    def _check_hand_over_user(self):
        self.ensure_one()
        payment_issue = self.mapped('payment_ids').filtered(
            lambda pay: pay.cdc_state != 'registered' and self.user_from_id != pay.cheque_owner_id)
        if payment_issue:
            raise ValidationError(
                _('Please remove below CDC(s) that does not registered or hand overed already: \n %(payments)s') % {
    ...
    def action_submit(self):
        ...
        template_param = 'account_cdc.default_notify_hand_over_template'
        for record in self:
            self.action_approve()
    def action_approve(self):
        """ update status and change owner of cdc """
        self.write({'state': 'hand_over'})
        for record in self:
            payment_issue = record.mapped('payment_ids').filtered(
                lambda pay: pay.cdc_state != 'registered' and record.user_from_id != pay.cheque_owner_id)
            if payment_issue:
                raise ValidationError(
                    _('You can not hand over below CDC(s) that does not registered or hand overed already, '
    ...
    def action_reject(self, reason=''):
        ...
        template_param = 'account_cdc.default_notify_hand_over_reject_template'
        for record in self:
    ...
    def action_open_cdc_receivable(self):
        """
        open related cdc receivable
        :return: action
        """

        payments = self.mapped('payment_ids')
        action = self.env["ir.actions.actions"]._for_xml_id("account_cdc.account_payment_cdc_receivable_action")
        if len(payments) > 1:
            action['domain'] = [('id', 'in', payments.ids)]
        elif len(payments) == 1:
            form_view = [(self.env.ref('account_cdc.account_payment_cdc_receivable_form_inherit_primary').id, 'form')]
            if 'views' in action:
