...
class AccountPaymentCash(models.TransientModel):
    ...
    def action_register_payment(self):
        ...
        for payment in payments:
            if payment.is_pdc_payment:
                ...
                if reconciled_invoice_ids:
                    vals = {
                        ...
                        'communication': 'Cheque Info ' + payment.name if payment.name else ' ' + ' - ' + payment.memo if payment.memo else ' ',
                    ...
                    vals = {
                        'amount': payment._get_payment_amount(company_currency=False),
                        'memo': f'Cheque Info {payment.name if payment.name else ""}  -  {payment.memo if payment.memo else "" }',
                        'currency_id': payment.currency_id.id,
                        'payment_type': 'inbound',
                        'partner_type': 'customer',
                        'partner_id': payment.partner_id.id,
                    ...
                    new_payment.move_id.cheque_payment_type = 'cashed'
            ...
            elif payment.is_cdc_payment:
                reconciled_invoice_ids = payment.reconciled_invoice_ids
                self._reverse_original_cdc(payment)
                payment.cdc_state = 'cashed'
                payment.move_id.mapped('line_ids').remove_move_reconcile()
                if reconciled_invoice_ids:
                    vals = {
                        'amount': payment._get_payment_amount(company_currency=False),
    ...
    def _reverse_original_cdc(self, payments):
        """
        reverse cdc journal entry
        """
        for payment in payments:
            if payment.cdc_state == 'registered':
                default_values_list = [{
                    'cheque_payment_id': payment.id,
                }]
