...
#         # ربط سطر الفاتورة بسطر SO عبر الجدول الوسيط (نحتاجه مبكرًا لبناء EXISTS)
#         m2m = self.env['account.move.line']._fields['sale_line_ids']
#         rel_table = m2m.relation
#         col_aml   = m2m.column1
#         col_sol   = m2m.column2

#         # بارامترات لاستخدامها في ids_sql فقط
#         so_params = []
...
#         # EXISTS لاختيار SO إذا لم نفلتر على تاريخ SO بل على تاريخ الفاتورة
...
#         # ⚠️ احتفظ بنسخة بارامترات ids_sql قبل إضافة بارامترات JOIN
#         so_params_ids = list(so_params)

#         # فلتر تواريخ الفاتورة ليُطبَّق على الـ JOIN نفسه (لتقييد السطور المعروضة)
...
#         # ===== جهّز خريطة Ship to Site عبر ORM (x_studio_site_) =====
#         ids_sql = f"""
#             SELECT so.id
#             FROM sale_order so
...
#         """
#         self.env.cr.execute(ids_sql, tuple(so_params_ids))
...
#         # ===== تجهيز JOINs وأعمدة أسماء العلاقات المطلوبة =====
#         resolve_fields = [
...
#                    'Journal Name','Invoice Status','Payment Status','Return Reason','RMA Status']
...
#         # ===== قسم أوامر البيع (نسخة "سطر لكل فاتورة") =====
#         so_sql_stream = f"""
#             WITH so_filtered AS (
#                 SELECT
...
#                 -- بيانات الفاتورة/سطر الفاتورة (كل سطر = صف)
#                 aml.id              AS aml_id,
#                 am.id               AS move_id,
...
#                 -- تصفير قيم الـ INV عند غياب الفاتورة
...
#                 -- شريك/عناوين + أعمدة الأسماء المطلوبة
...
#             -- اربط كل سطور الفاتورة المرتبطة بسطر الـ SO (بدون DISTINCT)
...
#         """
#         # بارامترات تنفيذ so_sql_stream = بارامترات ids_sql + بارامترات فلتر JOIN
#         so_params_stream = tuple(so_params_ids + inv_join_params)
...
#         # ===== معاملات وفلتر تاريخ إشعار الدائن (Credit Note) لقسم RMA =====
#         rma_params = []
#         rma_date_filter = ""
#         if self.invoice_date_from:
...
#                 -- Based on Product (rma_product_line)
#                 SELECT
#                     rb.*,
...
#                 -- سطر الفاتورة المقارب لنفس الإشعار الدائن (إن وُجد)
...
#             -- اختيار سطر AML الأنسب: طابق المنتج أولاً، ثم العبوة، ثم أقرب كمية
#             LEFT JOIN LATERAL (
#                 SELECT il.*
...
#             inv_sub_use = (r.get('inv_price_subtotal') or 0.0)  # RAW
#             inv_tot_use = (r.get('inv_price_total') or 0.0)  # RAW
#             inv_vat_use = max(inv_tot_use - inv_sub_use, 0.0)  # RAW
...
#             # قيَم RMA بالمطلق ثم إشارة حسب سطر الخصم
...
#             # قيَم الفاتورة دائمًا بالسالب في قسم RMA
#             sign_inv = -1
...
#                 rma_packaging_name,  # RMA_Product Packaging
#                 rma_pack_qty,        # RMA_Packaging Quantity
#                 rma_pack_price,

#                 inv_packaging_name,  # INV_Product Packaging
#                 inv_pack_qty,
#                 inv_pack_price,
...
#                 '',  # Invoiced Qty
#                 '',  # Cost
#                 excise_tax_val,  # Excise Tax

#                 inv_discount,
#                 inv_discount_amount,
#                 inv_gross_amount,

#                 rma_subtotal,      # RMA_Total Before VAT
#                 rma_vat_amount,    # RMA_VAT Amount
#                 rma_total_after,   # RMA_Total after VAT

#                 inv_sub_use,       # INV_Total Before VAT
#                 inv_vat_use,       # INV_VAT Amount
#                 inv_tot_use,       # INV_Total after VAT
...
class SalesOrderRmaReportWizard(models.TransientModel):
    ...
    def generate_report(self):
        ...
        # ربط سطر الفاتورة بسطر SO عبر الجدول الوسيط (للاستخدام في EXISTS)
        m2m = self.env['account.move.line']._fields['sale_line_ids']
        rel_table = m2m.relation
        col_aml   = m2m.column1
        col_sol   = m2m.column2
        ...
        # EXISTS على تواريخ الفواتير (إن لزم)
        invoice_date_subfilter = ""
        if (not self.show_invoice_dates) and (self.invoice_date_from or self.invoice_date_to):
            invoice_date_subfilter = f"""
                AND EXISTS (
        ...
        # نسخة معرّفات قبل بارامترات JOIN
        so_params_ids = list(so_params)

        # فلتر تواريخ الفاتورة على JOIN
        inv_join_date_filter = ""
        inv_join_params = []
        if self.invoice_date_from:
            inv_join_date_filter += " AND am.invoice_date >= %s"
        ...
        # ===== تجهيز JOINs لأعمدة العلاقات (اختياري) =====
        resolve_fields = [
            ('partner_channel_id',      'channel_name'),
            ('outlet_id',               'outlet_class_name'),
            ('sub_outlet_id',           'sub_class_name'),
        ...
        so_sql_stream = f"""
            ...
            SELECT
                ...
                -- الشريك وعناوينه + Ship to Site (من partner_shipping فقط)
                p.name AS partner_name, p.customer_code, p.partner_channel_id, p.outlet_id,
                p.sub_outlet_id, p.customer_type, p.customer_group_id, p.outlet_code,
                p.customer_subdivision_id, p.external_ref,
        ...
        for r in dictfetchmany(self.env.cr, FETCH_BATCH):
            ...
            _row_vals = [
                ...
                r.get('sub_channel_name') or '',
                r.get('external_ref') or '',
                ship_to_site,  # Ship to Site (من partner_shipping فقط)
                r.get('so_po') or '',
                r.get('assign_partner_name') or '',
                so_day,
                r.get('product_name') or '',
        ...
        # ===== معاملات وفلتر تاريخ إشعار الدائن (RMA) =====
        rma_params = []
        rma_date_filter = ""
        if self.invoice_date_from:
            rma_date_filter += " AND cn.invoice_date >= %s"
        ...
        rma_sql = f"""
            ...
            SELECT
                ...
                -- بيانات العميل + Ship to Site (من partner_shipping فقط)
                p.name AS partner_name, p.customer_code, p.partner_channel_id, p.outlet_id,
                p.sub_outlet_id, p.customer_type, p.customer_group_id, p.outlet_code,
                p.customer_subdivision_id, p.external_ref,
        ...
        for r in dictfetchmany(self.env.cr, FETCH_BATCH):
            ...
            inv_price_subtotal  = (r.get('inv_price_subtotal') or 0.0)
            inv_price_total     = (r.get('inv_price_total') or 0.0)
            inv_vat_use         = max(inv_price_total - inv_price_subtotal, 0.0)
            ...
            EPS = 1e-6
            has_aml = bool(r.get('il_product_id'))
            inv_amount_for_sign = inv_price_subtotal if abs(inv_price_subtotal) > EPS else inv_price_total
            is_discount = has_aml and (inv_amount_for_sign < -EPS)
            ...
            # قيم الفاتورة بالسالب في قسم RMA
            sign_inv = -1
            inv_unit_qty *= sign_inv
            inv_pack_qty *= sign_inv
            inv_unit_price *= sign_inv
            inv_gross_amount *= sign_inv
            inv_discount_amount *= sign_inv
            inv_price_subtotal *= sign_inv
            inv_price_total *= sign_inv
            inv_vat_use *= sign_inv
            excise_tax_val *= sign_inv
            inv_discount *= sign_inv
            ...
            _row_vals = [
                ...
                r.get('sub_channel_name') or '',
                r.get('external_ref') or '',
                safe_json(r.get('ship_site_number') or ''),  # Ship to Site (partner_shipping فقط)
                '',  # Customer Po Reference
                r.get('salesman_name') or '',
                rma_day,
                r.get('product_name') or '',
                ...
                inv_price_subtotal,       # INV_Total Before VAT
                inv_vat_use,              # INV_VAT Amount
                inv_price_total,          # INV_Total after VAT
